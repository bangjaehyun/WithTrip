...
// 카테고리 검색 객체를 생성합니다
var cps = new kakao.maps.services.Places(map);
...
function searchPlaces(bool) {
	...
	if (bool) {
		// 장소검색 객체를 통해 카테고리로 장소검색을 요청합니다
		cps.keywordSearch(keyword, placesSearchCB, {
			useMapBounds: true
		});
	} else {
...
function getListItem(index, places) {
	...
	if (places.road_address_name) {
		...
		itemStr += '<span>-</span>' + '<span class="jibun gray">'
...
function toggleShow() {
	...
	$("#toggleListBtn").css("background-image", "url(/resources/images/map_menu_close.svg)")
}

function toggleHide() {
	$("#toggleListBtn").val("0");
	$("#toggleListBtn").css("transform", "translateX(-330px)");
	$("#toggleListBtn").css("background-image", "url(/resources/images/map_menu_open.svg)")
}
