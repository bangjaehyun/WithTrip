...
public class FestivalService {
	...
	/*
	 * public ArrayList<FestivalMain> festivalMain(String toDay, String lastDay,
	 * String pageNo) {
	 * 
	 * // 1. URL을 만들기 위한 StringBuilder. StringBuilder urlBuilder = new
	 * StringBuilder( "http://apis.data.go.kr/B551011/KorService1/searchFestival1");
	 * 
	 * BufferedReader rd = null; HttpURLConnection conn = null; StringBuilder sb =
	 * new StringBuilder(); try { urlBuilder.append( "?" +
	 * URLEncoder.encode("eventStartDate", "UTF-8") + "=" + URLEncoder.encode(toDay,
	 * "UTF-8")); // 시작일 urlBuilder.append( "&" + URLEncoder.encode("eventEndDate",
	 * "UTF-8") + "=" + URLEncoder.encode(lastDay, "UTF-8")); // 종료일
	 * urlBuilder.append("&areaCode=&sigunguCode="); // 2. 오픈 API의요청 규격에 맞는 파라미터 생성,
	 * 발급받은 인증키. urlBuilder.append("&" + URLEncoder.encode("ServiceKey", "UTF-8") +
	 * "=" + URLEncoder.encode(
	 * "AfQfhsnbGileldKwUWQ+BO/64/UexpjEbEYOo6OQNcKt8jfPaOeYcnuLn+I44uDmsfXhYgjRlkPAfbIl/suZxw==",
	 * "UTF-8")); urlBuilder.append(
	 * "&listYN=Y&MobileOS=ETC&MobileApp=AppTest&arrange=A&numOfRows=12");
	 * 
	 * // 페이지를 불러올 때 사용할 문자열 저장
	 * 
	 * urlBuilder.append("&" + URLEncoder.encode("pageNo", "UTF-8") + "=" +
	 * URLEncoder.encode(pageNo, "UTF-8"));
	 * 
	 * // 3. URL 객체 생성. URL url = new URL(urlBuilder.toString()); // 4. 요청하고자 하는
	 * URL과 통신하기 위한 Connection 객체 생성. conn = (HttpURLConnection)
	 * url.openConnection(); // 5. 통신을 위한 메소드 SET. conn.setRequestMethod("GET"); //
	 * 6. 통신을 위한 Content-type SET. conn.setRequestProperty("Content-type",
	 * "application/json"); // 7. 통신 응답 코드 확인. System.out.println("Response code: "
	 * + conn.getResponseCode()); // 8. 전달받은 데이터를 BufferedReader 객체로 저장.
	 * 
	 * if (conn.getResponseCode() >= 200 && conn.getResponseCode() <= 300) { rd =
	 * new BufferedReader(new InputStreamReader(conn.getInputStream())); } else { rd
	 * = new BufferedReader(new InputStreamReader(conn.getErrorStream())); } // 9.
	 ...
	 * TODO Auto-generated catch block e.printStackTrace(); } finally { try { // 10.
	 * 객체 해제. rd.close(); conn.disconnect(); } catch (IOException e) { // TODO
	 ...
	 * DocumentBuilder builder = factory.newDocumentBuilder();
	 * System.out.println(sb.toString()); InputStream is = new
	 * ByteArrayInputStream(sb.toString().getBytes()); Document doc =
	 * builder.parse(is); NodeList nodes = doc.getElementsByTagName("item");
	 * 
	 * for (int i = 0; i < nodes.getLength(); i++) { Node nNode = nodes.item(i);
	 * Element element = (Element) nNode;
	 * 
	 * FestivalMain festival = new FestivalMain();
	 * festival.setFestivalId(element.getElementsByTagName("contentid").item(0).
	 * getTextContent());
	 * festival.setFestivalType(element.getElementsByTagName("contenttypeid").item(0
	 * ).getTextContent());
	 * festival.setFestivalImage(element.getElementsByTagName("firstimage").item(0).
	 * getTextContent());
	 * festival.setFestivalTitle(element.getElementsByTagName("title").item(0).
	 * getTextContent());
	 * festival.setFestivalPageNo(doc.getElementsByTagName("pageNo").item(0).
	 * getTextContent());
	 * festival.setFestivalTotalCount(doc.getElementsByTagName("totalCount").item(0)
	 * .getTextContent()); list.add(festival); } } catch (Exception e) {
	 * e.printStackTrace(); }
	 * 
	 * return list; }
	 */
